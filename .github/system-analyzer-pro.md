...
* **Usecase:** Phải có ranh giới hệ thống (`subgraph`), phân biệt rõ Actor chính (Admin/User) và Actor hệ thống (asset/AI). Sử dụng đúng `-->` cho tương tác và `-.->` cho `<<include>>/<<extend>>`.
...
* **Phân loại:** [Tên Usecase] -> [Loại DB] -> [Vùng hiển thị (asset/Incident/Admin)].
