...
* **Backend Stack:** Go (Golang) sử dụng mô hình Hybrid Database:
    * **PostgreSQL (GORM):** Quản lý quan hệ (Users, Organizations, Agents, Incidents, Approvals).
...
* **Kiểm tra Tương thích:** * Kiểm tra sự tương thích giữa GORM (Postgres) và MongoDB (đảm bảo `agent_hw_id` luôn khớp).
...
* **Real-time:** Ưu tiên sử dụng `websocket.GlobalHub.Broadcast` để thông báo trạng thái cập nhật Agent.
...
* **Database Optimization:**
    ...
    * **Batching:** Ưu tiên các lệnh `InsertMany` hoặc `UpdateMany` khi xử lý dữ liệu Baseline hoặc Telemetry từ Agent.
* **React Rendering:** Sử dụng `React.memo`, `useMemo`, và `useCallback` cho các Component hiển thị danh sách lớn (như bảng Agents) để tránh re-render không cần thiết.

## 6. Quy tắc Bảo mật Tuyệt đối (Security-First)
Vì đây là hệ thống SENT-SYSTEM, an ninh là ưu tiên số 1:
* **Input Validation:** Mọi dữ liệu từ Agent gửi lên (`AgentPayload`) phải được kiểm tra tính hợp lệ trước khi xử lý.
* **Authentication & Authorization:** * Kiểm tra `Status == 'ACTIVE'` của Agent trước khi chấp nhận Log.
