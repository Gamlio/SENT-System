    ...
    return (
        <div className="text-slate-200 relative">
            ...
            <div className="grid grid-cols-1 xl:grid-cols-3 gap-8">
                ...
                {/* --- CỘT PHẢI: TÌM KIẾM, DANH SÁCH & PHÂN TRANG --- */}
                <div className="xl:col-span-2 flex flex-col h-full">
                    ...
                    <div className="space-y-4 flex-1">
                            ...
                            <div key={doc.ID} className="bg-[#1e293b] p-5 rounded-2xl border border-slate-800 flex justify-between items-center group hover:border-slate-700 transition">
                                <div className="flex gap-4 items-center">
                                    ...
                                    <div>
                                        ...
                                        <span className="text-[10px] px-2 py-0.5 bg-slate-800 text-slate-400 rounded uppercase font-bold mr-2">{doc.category}</span>
                                        <span className="text-[10px] text-slate-500"><Clock size={10} className="inline mr-1"/>{new Date(doc.CreatedAt).toLocaleDateString()}</span>
                                    </div>
                                </div>
                                <div className="flex items-center gap-4 sm:gap-6">
                                    {doc.is_processed ? (
                                        <span className="hidden sm:flex items-center gap-1 text-[10px] font-bold text-emerald-400 bg-emerald-500/10 px-3 py-1 rounded-full"><CheckCircle size={12}/> AI READY</span>
                                    ) : (
                                        <span className="hidden sm:flex items-center gap-1 text-[10px] font-bold text-amber-400 bg-amber-500/10 px-3 py-1 rounded-full"><RefreshCw size={12} className="animate-spin"/> PENDING</span>
                                    )}
                                    
                                    <div className="flex gap-2 opacity-100 sm:opacity-0 sm:group-hover:opacity-100 transition-opacity">
                                        {/* Sửa lại hàm onClick của nút Xem để truyền cả object doc vào */}
                        ...
                        {filteredDocs.length === 0 && (
                            <div className="text-center py-20 text-slate-600 border-2 border-dashed border-slate-800 rounded-3xl">Không tìm thấy tài liệu nào phù hợp.</div>
                        )}
                    </div>
            ...
            {/* Modal Xem Tài Liệu Trực Tiếp Trên Trang */}
            {viewingDoc && (
                    ...
                    <div className="bg-[#1e293b] rounded-3xl border border-slate-700 w-full max-w-5xl h-[90vh] flex flex-col shadow-2xl overflow-hidden">
                        ...
                        <div className="p-5 border-b border-slate-700 flex justify-between items-center bg-slate-800/50">
                            ...
                            <button 
                                ...
                                className="p-2 bg-slate-700 hover:bg-red-500/80 text-slate-300 hover:text-white rounded-xl transition"
                                title="Đóng"
                            >
                                <X size={20}/>
                            </button>
                        ...
                        <div className="flex-1 bg-white relative">
                            {viewingDoc.file_name.endsWith('.pdf') ? (
                                <iframe
                                ...
                                <div className="flex flex-col items-center justify-center h-full bg-slate-100">
                                    ...
                                    <p className="text-slate-600 font-bold mb-2">Định dạng Word không hỗ trợ xem trực tiếp trên trình duyệt.</p>
                                    <p className="text-slate-500 text-sm mb-6">Vui lòng tải xuống hoặc chuyển đổi sang định dạng PDF để xem trực tiếp.</p>
                                    <button 
                                        onClick={() => handleDownloadDocument(viewingDoc.file_path, viewingDoc.file_name)}
                                        className="bg-emerald-500 hover:bg-emerald-600 text-white px-6 py-3 rounded-xl font-bold flex items-center gap-2 transition"
                                    >
                                        <Download size={18}/> Tải xuống máy
                                    </button>
                                </div>
                            )}
                        </div>
            ...
            {editingDoc && (
                <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-50">
                    <div className="bg-[#1e293b] p-8 rounded-3xl border border-slate-700 w-full max-w-md shadow-2xl">
                        <h2 className="text-xl font-bold text-white mb-6">Cập nhật tài liệu</h2>
                        <form onSubmit={(e) => { e.preventDefault(); updateDoc(editingDoc.ID, {title: editingDoc.title, category: editingDoc.category}); }} className="space-y-4">
                            <div>
                                <label className="text-xs font-bold text-slate-500 uppercase">Tiêu đề</label>
                                <input type="text" value={editingDoc.title} onChange={(e) => setEditingDoc({...editingDoc, title: e.target.value})} className="w-full mt-2 p-3 bg-slate-900 border border-slate-700 rounded-xl outline-none focus:border-blue-500 text-white" />
                            </div>
                            <div>
                                <label className="text-xs font-bold text-slate-500 uppercase">Phân loại</label>
                                <select value={editingDoc.category} onChange={(e) => setEditingDoc({...editingDoc, category: e.target.value})} className="w-full mt-2 p-3 bg-slate-900 border border-slate-700 rounded-xl outline-none focus:border-blue-500 text-white">
                                    <option value="Internal">Quy định nội bộ</option>
                                    <option value="ISO27001">Tiêu chuẩn ISO 27001</option>
                                    <option value="Law">Luật An ninh mạng</option>
                                </select>
                            ...
                            <div className="flex gap-4 mt-8">
                                <button type="button" onClick={() => setEditingDoc(null)} className="flex-1 py-3 text-slate-400 bg-slate-800 hover:bg-slate-700 rounded-xl font-bold transition">Hủy</button>
                                <button type="submit" className="flex-1 py-3 text-white bg-blue-500 hover:bg-blue-600 rounded-xl font-bold transition shadow-lg shadow-blue-500/20">Lưu thay đổi</button>
                            </div>
                        </form>
                    </div>
                </div>
