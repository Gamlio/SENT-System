...
const VersionCard = ({ version }) => {
    ...
    return (
        <div className={`group relative bg-slate-900/50 backdrop-blur-sm border ${version.is_latest ? 'border-indigo-500/40' : 'border-slate-800'} rounded-2xl p-6 hover:border-indigo-500/60 transition-all duration-300`}>
            ...
            <div className="flex justify-between items-start mb-6">
                <div>
                    <div className="flex items-center gap-3">
                        <span className="text-white font-black text-xl tracking-tighter uppercase">Build {version.tag}</span>
                        {version.is_latest && (
                    ...
                    <div className="flex items-center gap-4 mt-2 text-slate-500 text-[10px] font-bold uppercase tracking-widest">
                        <span className="flex items-center gap-1.5"><Calendar size={12}/> {version.release_date}</span>
                    </div>
                </div>
            </div>
