...
const Agents = () => {
    ...
    const {
        ...
        initiateBulkDelete
    } = useAgentBulkActions(fetchAgents);
        ...
        <div className="text-slate-200 pb-20 relative">
            ...
            <div className="mb-6 flex flex-col md:flex-row gap-4 mt-6">
                ...
                <div className="relative min-w-[220px]">
                     <div className="absolute left-4 top-1/2 -translate-y-1/2 text-emerald-500 pointer-events-none"><ArrowUpDown size={18} /></div>
                     <select value={`${sortConfig.key}-${sortConfig.direction}`} onChange={handleSortChange} className="w-full pl-11 pr-8 py-3 bg-[#1e293b] border border-slate-800 rounded-2xl text-white outline-none focus:border-emerald-500 appearance-none cursor-pointer font-bold text-sm shadow-lg hover:bg-slate-800 transition">
            ...
            <div className="bg-[#1e293b] rounded-3xl border border-slate-800 shadow-xl overflow-hidden flex flex-col min-h-[600px]">
                    ...
                    <table className="w-full text-left whitespace-nowrap">
                            ...
                            <tr>
                                <th className="p-5 w-10">
                                    <input 
                                        ...
                                        onChange={(e) => handleSelectAll(currentAgents, e.target.checked)} 
                                        className="w-4 h-4 rounded bg-slate-800 border-slate-700 accent-emerald-500 cursor-pointer"
                                    />
                                </th>
                                <th className="p-5 font-bold cursor-pointer hover:text-emerald-400 transition" onClick={() => setSortConfig({ key: 'hostname', direction: sortConfig.direction === 'asc' ? 'desc' : 'asc' })}>Máy trạm (Hostname)</th>
                                <th className="p-5 font-bold">Người chịu trách nhiệm</th>
                                <th className="p-5 font-bold cursor-pointer hover:text-emerald-400 transition" onClick={() => setSortConfig({ key: 'ip_address', direction: sortConfig.direction === 'asc' ? 'desc' : 'asc' })}>Mạng (IP)</th>
                        ...
                        <tbody className="divide-y divide-slate-800">
                            {currentAgents.length === 0 ? (
                                <tr><td colSpan="6" className="p-10 text-center text-slate-500 italic">Không tìm thấy máy trạm nào phù hợp.</td></tr>
                            ) : (
                                currentAgents.map(agent => (
                                    <tr key={agent.hwid} className={`hover:bg-slate-800/50 transition-colors group ${selectedAgents.includes(agent.hwid) ? 'bg-emerald-500/5' : ''}`}>
                                        ...
                                        {/* ... (Các cột hiển thị dữ liệu giữ nguyên) ... */}
                                        <td className="p-5">
                                        ...
                                        <td className="p-5">
                                            <div className="flex flex-col group/manager relative">
                                                <div className="flex items-center gap-2 text-sm text-slate-300"><User size={14} className="text-blue-400"/> {agent.manager?.full_name || "Chưa bàn giao"}</div>
                                                <div className="flex items-center gap-2 text-xs text-slate-500"><Smartphone size={12}/> {agent.manager?.phone || "---"}</div>
                                                <button onClick={() => { setTargetAgent(agent); setAssignModalOpen(true); }} className="text-[10px] text-emerald-500 hover:text-emerald-400 font-bold mt-1 opacity-0 group-hover:opacity-100 transition-opacity text-left">+ Thay đổi người quản lý</button>
                                            </div>
                                        </td>
                ...
                {/* --- PHÂN TRANG GOM GỌN --- */}
                {totalPages > 1 && (
                     <div className="p-4 bg-slate-900/80 border-t border-slate-800 flex justify-between items-center gap-4">
                        <div className="text-xs text-slate-500 font-bold uppercase"><LayoutList size={14} className="inline mr-2"/>Trang <span className="text-white">{currentPage}</span> / {totalPages}</div>
                        <div className="flex gap-2">
                            <button disabled={currentPage === 1} onClick={() => setCurrentPage(p => p - 1)} className="px-3 py-2 bg-slate-800 text-slate-400 hover:text-white rounded-xl disabled:opacity-30">Trước</button>
                            <button disabled={currentPage === totalPages} onClick={() => setCurrentPage(p => p + 1)} className="px-3 py-2 bg-slate-800 text-slate-400 hover:text-white rounded-xl disabled:opacity-30">Sau</button>
                        </div>
                     </div>
                )}
            </div>

            {/* --- THANH CÔNG CỤ NỔI (FLOATING BAR) DÀNH CHO XÓA HÀNG LOẠT --- */}
            {selectedAgents.length > 0 && (
                <div className="fixed bottom-10 left-1/2 -translate-x-1/2 bg-slate-800 text-white px-6 py-4 rounded-2xl shadow-2xl border border-emerald-500/50 flex items-center gap-6 z-50 animate-in slide-in-from-bottom-5 duration-300">
                    <span className="text-sm font-bold"><span className="bg-emerald-500 px-2.5 py-1 rounded-lg mr-2 text-xs font-black">{selectedAgents.length}</span> máy</span>
                    
                    <button 
                        onClick={initiateBulkDelete} 
                        className="flex items-center gap-2 bg-red-500/10 hover:bg-red-500/20 text-red-400 border border-red-500/30 px-5 py-2.5 rounded-xl transition font-bold text-xs uppercase"
                    >
                        <Trash2 size={16}/> Gỡ bỏ
                    </button>

                    <button onClick={clearSelection} className="p-2 text-slate-400 hover:text-red-400"><X size={20}/></button>
                </div>
            )}
{assignModalOpen && (
            ...
            {/* --- HỘP THOẠI (DIALOG) XÓA HÀNG LOẠT --- */}
            <AppDialog 
                {...dialogConfig}
                inputValue={deleteReason}
                onInputChange={setDeleteReason}
                onConfirm={dialogConfig.onConfirm ? () => dialogConfig.onConfirm(deleteReason) : undefined} 
                onClose={closeDialog}
            />
