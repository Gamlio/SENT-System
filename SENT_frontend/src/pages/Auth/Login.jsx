...
const Login = () => {
    ...
    // THÊM company_code VÀO STATE
    const [form, setForm] = useState({ company_code: '', username: '', password: '', captcha: '', otp: '' });
    const { login } = useAuth();
                    ...
                    {step === 1 ? (
                        <form onSubmit={handleNext} className="space-y-6">
                           ...
                           <div className="space-y-2">
                                <label className="text-xs font-bold text-slate-400 uppercase tracking-widest ml-1">Mã công ty</label>
                                <div className="relative">
                                    <Building2 className="absolute left-4 top-1/2 -translate-y-1/2 text-slate-500" size={18}/>
                                    {/* Xóa chữ required ở thẻ input bên dưới */}
                                    <input type="text"
                                        className="w-full p-4 pl-12 bg-slate-900/50 rounded-2xl border border-slate-700 text-white focus:ring-2 focus:ring-emerald-500 outline-none transition-all uppercase"
                                        placeholder="VD: FPT (KHÔNG BẮT BUỘC)"
