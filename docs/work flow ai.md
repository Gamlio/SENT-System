...
Bắt đầu từ CLAUDE. md và các rules luôn được nạp vào context, đặt ra delivery standards rất cụ thể: truth hơn speed, tự verify trước khi tuyên bố hoàn thành, không dùng các câu như “should be fine”, “probably passes”, và phải có rollback path. Tức là agent không được phép chỉ “có vẻ đúng”, mà phải làm việc như một người có kỷ luật vận hành. 
Đây là bài học đầu tiên khi xây skills/agent cho cá nhân hoặc doanh nghiệp: muốn agent tốt, phải dạy nó cách làm việc trước khi dạy nó làm việc gì?
Rất nhiều đội ngũ hiện nay xây agent theo kiểu: có một prompt vai trò, thêm vài knowledge file, xong. Cách đó thường tạo ra một “AI biết nhiều”, nhưng không tạo ra một “AI làm việc đáng tin”. Bởi điều thiếu không phải là tri thức, mà là luật hành vi.
...
khi nào agent được truy cập dữ liệu nào,
...
Điều đáng chú ý là: doanh nghiệp không nên bắt đầu bằng “agent lớn”, mà nên bắt đầu bằng “skill nhỏ nhưng đáng tin”.
Bởi agent lớn thường ấn tượng trong demo nhưng khó kiểm soát trong thực tế. Skill nhỏ thì dễ test, dễ đo, dễ cải tiến, và dễ ghép thành hệ thống sau này.
4. Lớp Tác tử: agent không phải “AI biết tuốt”, mà là vai trò chuyên môn hóa
agents/ với các agent chuyên vai như pr-reviewer, security-reviewer, performance-analyzer. Ý nghĩa của nó rất rõ: thay vì kỳ vọng một AI làm mọi thứ, tác giả tách theo chuyên môn và góc nhìn kiểm tra. 
Đây là điểm nhiều đội ngũ bỏ qua khi xây agent doanh nghiệp. Họ muốn một “siêu trợ lý” biết mọi thứ từ kế toán đến sales đến pháp lý. Kết quả là agent trả lời cái gì cũng được, nhưng cái gì cũng nông.
Cách đúng hơn là thiết kế agent như các vai trong một tổ chức.
Với cá nhân, anh có thể có các agent như:
• Agent nghiên cứu thị trường
• Agent kiến trúc giải pháp
• Agent viết proposal
• Agent review logic
• Agent quản lý tri thức
• Agent chuẩn bị họp / follow-up
Với doanh nghiệp, mô hình tác tử nên gần với cấu trúc phòng ban hoặc chức năng:
• Agent tiếp nhận yêu cầu
• Agent phân loại và định tuyến
• Agent thực thi theo SOP
• Agent kiểm tra tuân thủ
• Agent tổng hợp báo cáo
• Agent escalation sang con người
Quan trọng nhất: agent không nên được định nghĩa theo tên hay phong cách, mà theo trách nhiệm và ranh giới quyết định.
Một agent tốt cần biết 4 điều:
. Tôi chịu trách nhiệm phần nào.
. Tôi không được làm phần nào.
. Khi nào tôi phải gọi skill nào.
. Khi nào tôi phải chuyển tiếp cho agent khác hoặc con người.
Khi thiết kế như vậy, hệ agent bắt đầu giống một tổ chức thu nhỏ. Và lúc đó AI không còn là một chatbot, mà trở thành một hệ vai trò có điều phối.
5. Lớp Kiểm chứng: đây là cổng biến AI từ “thuyết phục” thành “đáng tin”
Skill verification-before-completion của repo có một triết lý rất mạnh: claiming work is complete without verification is dishonesty, not efficiency. Nó buộc agent trước khi khẳng định phải xác định lệnh kiểm chứng nào chứng minh được claim, chạy lệnh đó, đọc output đầy đủ, kiểm exit code và chỉ sau đó mới được kết luận. Repo cũng chỉ ra các red flags như dùng từ “should”, “probably”, tin báo cáo từ agent khác, hoặc dựa vào kiểm tra một phần. 
...
Nói cách khác, doanh nghiệp không nên chỉ hỏi “agent tạo ra được gì”, mà phải hỏi “agent được phép công bố điều gì sau khi đã qua cổng nào”.
...
Nếu hôm nay agent làm xong việc mà ngày mai lại không nhớ gì, thì hệ thống đó chưa tiến hóa. Nó chỉ đang “tiêu hao token để tạo output”.
...
Một tổ chức biết cách học từ agent sẽ ngày càng mạnh.
Một tổ chức chỉ dùng agent để trả lời nhanh sẽ mãi dậm chân ở mức tool usage.
...
Cuối cùng mới đến lớp agent. Lúc này agent chỉ là lớp vỏ điều phối trên các skill đó. Ví dụ:
• Agent tư vấn chiến lược
• Agent thiết kế chương trình đào tạo
• Agent biên tập tài liệu
• Agent review chất lượng
• Agent quản lý tri thức dự án
Cách làm này giúp bạn không sa vào bẫy “xây agent hoành tráng nhưng rỗng ruột”.
Ta xây từ skill trước, rồi mới dựng agent.
...
Lúc đó doanh nghiệp mới thật sự có “nền tảng agent”, chứ không chỉ có “một vài chatbot hay”.
Muốn xây skills/agent hiệu quả, đừng bắt đầu từ prompt. Hãy bắt đầu từ kỷ luật vận hành.
